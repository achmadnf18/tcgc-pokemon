<script>
  ...
  export let rarity = 'Rare Holo VMAX';
  ...
  const popover = () => {
    const rect = thisCard.getBoundingClientRect(); // get element's size/position
    let delay = 100;
    let scaleW = (window.innerWidth / rect.width) * 0.7;
    let scaleH = (window.innerHeight / rect.height) * 0.7;
    let scaleF = 1.75;
    setCenter();
    if (firstPop) {
      delay = 1000;
  ...
  <div class="card__translater">
    ...
    >
      ...
      <div class="card__front">
        <picture>
          <source src-set="{img.startsWith('http') ? '' : base}{img}" type="image/png" />
          <img
